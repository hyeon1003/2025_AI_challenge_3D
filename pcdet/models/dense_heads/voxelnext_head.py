...
class VoxelNeXtHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        # >>> ADDED: read class-wise loss weights from YAML (if provided)
        cls_w = None
        if hasattr(self.model_cfg, 'LOSS_CONFIG') and hasattr(self.model_cfg.LOSS_CONFIG, 'CLASS_LOSS_WEIGHTS'):
            cls_w = self.model_cfg.LOSS_CONFIG.CLASS_LOSS_WEIGHTS
        if cls_w is not None:
            self.register_buffer('class_loss_weights', torch.tensor(cls_w, dtype=torch.float32, device=self.point_cloud_range.device))
        else:
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            # >>> ADDED: class-wise weighted heatmap loss
            target_hm = target_dicts['heatmaps'][idx]   # shape: [num_voxels, C_head]
            C_head = pred_dict['hm'].shape[1]
            if self.class_loss_weights is not None:
                # 로컬(헤드) → 글로벌 클래스 인덱스 매핑
                global_ids = self.class_id_mapping_each_head[idx].long()  # shape: [C_head]
                head_weights = self.class_loss_weights[global_ids]        # shape: [C_head]
                hm_loss = 0.0
                # 클래스별로 손실 계산 후 가중합
                for c in range(C_head):
                    hm_loss_c = self.hm_loss_func(pred_dict['hm'][:, c:c+1], target_hm[:, c:c+1])
                    hm_loss = hm_loss + head_weights[c] * hm_loss_c
            else:
                hm_loss = self.hm_loss_func(pred_dict['hm'], target_hm)
            # <<< ADDED
