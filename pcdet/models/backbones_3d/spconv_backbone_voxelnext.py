...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        # 추가한 부분
        self.proj5 = spconv.SparseSequential(
            spconv.SubMConv3d(channels[4], channels[3], kernel_size=1, bias=False, indice_key='proj5'),
            norm_fn(channels[3]),
            nn.ReLU()
        )
        self.proj6 = spconv.SparseSequential(
            spconv.SubMConv3d(channels[4], channels[3], kernel_size=1, bias=False, indice_key='proj6'),
            norm_fn(channels[3]),
            nn.ReLU()
        )
        # 끝
    ...
    def forward(self, batch_dict):
        ...
        # (A) 채널 정합: 320 → 256으로 다운프로젝션
        x_conv5 = self.proj5(x_conv5)   # now C=256
        x_conv6 = self.proj6(x_conv6)   # now C=256
        x_conv5.indices[:, 1:] *= 2
        x_conv6.indices[:, 1:] *= 4
