...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    if rank == 0:
        ...
        from collections import defaultdict   # [ADD] 에폭 평균 통계를 위한 버퍼
        pbar = _tqdm.tqdm(total=total_it_each_epoch, leave=leave_pbar, desc='train', dynamic_ncols=True)
        data_time = common_utils.AverageMeter()
        batch_time = common_utils.AverageMeter()
        forward_time = common_utils.AverageMeter()
        losses_m = common_utils.AverageMeter()
        epoch_stats = defaultdict(list)       # [ADD] 에폭 평균용 누적 버퍼 (tb_dict 항목들)
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        # [ADD] 에폭 통계 누적 (rank==0에서만 사용)
        if rank == 0:
            if isinstance(tb_dict, dict):
                for k, v in tb_dict.items():
                    try:
                        epoch_stats[k].append(float(v))
                    except Exception:
                        # 텐서/넘파이/스칼라 모두 안전 변환
                        try:
                            epoch_stats[k].append(float(getattr(v, 'item', lambda: v)()))
                        except Exception:
                            pass
    ...
    # -----------------------------
    # [ADD] 에폭 단위 스칼라 기록 (x축=epoch로 보이게)
    # -----------------------------
    if rank == 0 and tb_log is not None:
        # 에폭 평균 loss/학습률
        try:
            tb_log.add_scalar('epoch/loss', losses_m.avg, cur_epoch + 1)
        except Exception:
            pass
        try:
            tb_log.add_scalar('epoch/lr',   cur_lr,        cur_epoch + 1)
        except Exception:
            pass
        # tb_dict 항목들의 에폭 평균
        try:
            for k, vals in epoch_stats.items():
                if len(vals) > 0:
                    tb_log.add_scalar(f'epoch/{k}', sum(vals)/len(vals), cur_epoch + 1)
        except Exception:
            pass
